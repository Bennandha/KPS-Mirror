...
Check all yt-dlp api options from this <a href='https://github.com/yt-dlp/yt-dlp/blob/master/yt_dlp/YoutubeDL.py#L184'>FILE</a> or use this <a href='https://t.me/mltb_official/177'>script</a> to convert cli arguments to api options.
        ...
        if await aiopath.exists(rclone_path):
            buttons.ibutton("Delete rcl.conf", f"userset {user_id} drcc")
        buttons.ibutton("Back", f"userset {user_id} back")
        buttons.ibutton("Close", f"userset {user_id} close")
        await editMessage(message, 'Send rcl.conf. Timeout: 60 sec', buttons.build_menu(1))
...
Examples:
1. <code>{escape('<b>@KPSTorrent</b>')}</code> 
This will give output of:
<b>@KPSTorrent</b>  <code>69MB.bin</code>.

2. <code>{escape('<code>@KPSTorrent</code>')}</code> 
This will give output of:
<code>@KPSTorrent</code> <code>69MB.bin</code>.
